/**
 * Imagen para compartir el enlace (WhatsApp, redes), 1200x630, generada del
 * hero en cada build.
 *
 * El hero es vertical: va a lo alto y centrado sobre fondo crema. Así el
 * recorte cuadrado que usa WhatsApp en la miniatura cae sobre el templo.
 */
import type { APIRoute } from 'astro';
import sharp from 'sharp';
import fotoHero from '../assets/hero-parroquia.jpg';
import { COLOR_CREMA, IMAGEN_OG } from '../lib/seo';
...
export const GET: APIRoute = async () => {
  ...
  const imagen = await sharp({ create: { width: ancho, height: alto, channels: 3, background: COLOR_CREMA } })
